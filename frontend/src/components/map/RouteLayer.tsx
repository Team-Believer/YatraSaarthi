...
function safeRemoveLayer(map: mapboxgl.Map | null, layerId: string) {
  try {
...
function safeRemoveSource(map: mapboxgl.Map | null, sourceId: string) {
  try {
...
export const RouteLayer: React.FC<RouteLayerProps> = ({
  ...
  useEffect(() => {
    ...
    // Clean up previous alt layers/sources safely
    for (let i = 0; i < 5; i++) {
      safeRemoveLayer(map, `${altLayerPrefix}${i}`);
      safeRemoveSource(map, `${altSourcePrefix}${i}`);
    }
    ...
    try {
      ...
      console.warn('[RouteLayer] Alternative route layer warning:', err);
    }
  ...
  useEffect(() => {
    ...
    if (geometry.length < 2) {
      ...
      safeRemoveLayer(map, casingLayerId);
    ...
    try {
      ...
      console.warn('[RouteLayer] Render active route warning:', err);
    }
