...
export const NavStatusDrawer: React.FC<NavStatusDrawerProps> = ({
  ...
  let statusBadgeClass = 'bg-canvas-soft text-ink border border-border-clean';
  let statusBgClass = 'bg-canvas-soft border-border-clean text-ink';
  let statusIcon = <ShieldCheck className="w-5 h-5 text-ink" />;
  ...
  if (locPermission === 'denied') {
    ...
    statusBadgeClass = 'bg-canvas-soft text-ink border border-border-clean animate-pulse';
    statusBgClass = 'bg-canvas-soft border-border-clean text-ink';
    statusIcon = <Activity className="w-5 h-5 text-ink" />;
    statusDesc = 'Connecting to navigation telemetry';
  } else if (isRecovering) {
    statusTitle = 'GNSS recovering';
    statusBadge = 'Recovering';
    statusBadgeClass = 'bg-cyan-100 text-cyan-800 border border-cyan-200 animate-pulse';
    statusBgClass = 'bg-cyan-50 border-cyan-200 text-cyan-950';
    statusIcon = <RotateCw className="w-5 h-5 text-cyan-600 animate-spin" />;
    statusDesc = 'Validating GNSS fix';
  } else if (isDrActive) {
    ...
    statusDesc = 'Continuing navigation without satellite signal';
  } else if (isDegraded) {
  ...
  let accuracyClass = 'text-ink-mute font-semibold text-xl';

  if (isStandby) {
    accuracyDisplay = 'Standby';
    accuracySubtext = 'Available during navigation';
    accuracyClass = 'text-ink-mute font-semibold text-xl';
  } else if (isStarting || !hasTelemetry) {
  ...
  const getEnvironmentDisplay = () => {
    if (isStandby) return { text: 'Standby', className: 'text-ink-mute' };
  ...
  const getGnssSignalDisplay = () => {
    ...
    return { text: 'Standby', className: 'text-ink-mute' };
  };
  ...
  const getNhcDisplay = () => {
    if (isStandby || !hasTelemetry) return { text: 'Standby', className: 'text-ink-mute' };
  ...
  const drawerContent = (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/40 backdrop-blur-xs transition-opacity animate-in fade-in duration-200">
      ...
      <div
        ...
        className={clsx(
          'relative w-full sm:max-w-lg max-h-[85vh] bg-white rounded-t-3xl sm:rounded-2xl shadow-nav-modal border border-border-clean z-10 flex flex-col overflow-hidden transition-all animate-in slide-in-from-bottom-4 duration-200'
        )}
      >
        ...
        <div className="shrink-0 p-5 sm:p-6 pb-4 border-b border-border-clean flex items-center justify-between bg-white">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-full bg-canvas-soft text-ink flex items-center justify-center shrink-0">
            ...
            <div>
              ...
              <p className="text-[11px] text-ink-body leading-none mt-0.5">
                Real-time telemetry and fusion diagnostics
              </p>
            </div>
          </div>
          <button
            ...
            className="w-8 h-8 rounded-full bg-canvas-soft hover:bg-surface-pressed text-ink transition-colors flex items-center justify-center cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 pt-4 space-y-4 select-none hide-scrollbar">
          {/* Current State Banner */}
          <div className={clsx('p-4 rounded-2xl border flex flex-col gap-1.5 transition-colors duration-200', statusBgClass)}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {statusIcon}
                <span className="font-semibold text-xs sm:text-sm tracking-tight">{statusTitle}</span>
              </div>
              <div className="flex items-center gap-2">
                {outageDuration > 0 && isDrActive && (
                  <span className="font-mono text-xs font-semibold text-amber-900 bg-amber-200/60 px-2 py-0.5 rounded-full border border-amber-300">
                    {formatOutage(outageDuration)}
                  </span>
                )}
                <span className={clsx('text-[10px] font-semibold px-2.5 py-0.5 rounded-full uppercase tracking-wider', statusBadgeClass)}>
                  {statusBadge}
                </span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {/* Confidence */}
            <div className="p-4 bg-canvas-soft rounded-2xl border border-border-clean flex flex-col justify-between min-h-[90px]">
            ...
            <div className="p-4 bg-canvas-soft rounded-2xl border border-border-clean flex flex-col justify-between min-h-[90px]">
              <div className="flex items-center justify-between text-ink-body text-xs font-medium">
                ...
                <Gauge className="w-3.5 h-3.5 text-ink" />
              </div>
          ...
          <div className="space-y-1 pt-1">
            <div className="text-[10px] font-bold text-ink-mute uppercase tracking-wider px-0.5 mb-1.5">
              System telemetry
            </div>

            <div className="divide-y divide-border-clean border-t border-b border-border-clean">
            ...
            {hasTelemetry && fusedPosition && (
              <div className="pt-2 px-0.5 flex items-center justify-between text-[11px] font-mono text-ink-mute">
